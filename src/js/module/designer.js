...
function dragula (initialContainers, options) {
  ...
  function set_number(id){
    var v_id = 1;
    $("#"+ id +"").find(".item-box").each(function(i){
      var d_id = $(this).attr("id");
      if(d_id == "" || d_id == undefined){
      }else{
        var d_num = $(this).attr("id").substring(d_id.indexOf("-")+1);
        if(Number(d_num) >= Number(v_id)){
          v_id = String(Number(d_num) + 1);
        }
      }
    });
    $("#"+ id +"").find(".item-box").each(function(i){
      $(this).attr("date-no",i+1);
      var g_id = $(this).attr("id");
      if(g_id == "" || g_id == undefined){
        $(this).attr("id", "all-"+v_id+"");
      }
    });
  }
